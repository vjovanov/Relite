  ...
  def main(args: Array[String]): Unit = {
    ...
    energy <- function() {
      ...
      return(sum(0.5 * body_mass *
               (body_vx * body_vx + body_vy * body_vy + body_vz * body_vz)) -  #here the program breaks, because of accessing vectors that were updated
           sum(q[upper.tri(q)]))
    }

    nbody <- function(args) {
      n = if (length(args)) as.integer(args[[1]]) else 1000L
      options(digits=9)  #this does nothing for now
      offset_momentum()
      cat(energy(), "\n")
      for (i in 1:n)
        advance(0.01)
